...
# and the version to download. Example: NAME=Sentinel VERSION=0.16.1
ARG NAME
ARG VERSION

LABEL org.opencontainers.image.authors="Sentinel Product Team <sentinel@hashicorp.com>"
...
RUN set -eux && \
    apk add --no-cache ca-certificates curl gnupg libcap openssl jq iputils git make && \
